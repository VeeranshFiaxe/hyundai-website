...
export default function CarDetailClient({ car }: { car: Car }) {
  ...
  return (
    <>
      <section className="bg-white pb-10 pt-10 lg:pb-14 lg:pt-16">
        <div className="container-px mx-auto max-w-[1400px]">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16">
            ...
            <Reveal delay={150} variant="slide-left">
              ...
              <dl className="mt-6 grid grid-cols-2 gap-x-4 gap-y-5 border-t border-border pt-6 sm:grid-cols-3">
                {[
                  ["Seating", car.seating],
                  ["Fuel", car.fuel],
                  ...
                  <div key={label} className={label === "Best for" ? "col-span-2 sm:col-span-2" : ""}>
      ...
      <section id="overview" className="scroll-mt-28 bg-bg-2 py-12 lg:py-16">
        <div className="container-px mx-auto grid max-w-[1400px] gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand">Buyer&apos;s overview</p>
            <h2 className="mt-2 font-display text-2xl font-bold text-text sm:text-3xl">Know what fits your life before you drive it.</h2>
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted sm:text-base">{detail.overview}</p>
            <p className="mt-4 text-sm font-semibold text-text">Ideal for: <span className="font-normal text-muted">{detail.idealFor}</span></p>
          </div>
          ...
          <div className="mt-7 grid gap-4 lg:grid-cols-[1.6fr_0.9fr]">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2">
              {gallery.map((image, index) => <button key={image.src} type="button" onClick={() => setGalleryIndex(index)} aria-pressed={index === galleryIndex} className={`group overflow-hidden rounded-lg border bg-bg-2 text-left transition-all ${index === galleryIndex ? "border-brand ring-2 ring-brand/15" : "border-border hover:border-brand"}`}>
                <Image src={image.src} alt="" width={360} height={200} sizes="(max-width: 640px) 50vw, 240px" className="h-24 w-full object-contain p-2 transition-transform duration-300 group-hover:scale-105" />
