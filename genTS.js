...
let tsCode = `export const locations: Location[] = [\n`;
data.forEach(loc => {
  tsCode += `  {\n`;
  tsCode += `    name: "${loc.name}",\n`;
  tsCode += `    type: "${loc.type}",\n`;
  tsCode += `    city: "${loc.city}",\n`;
  tsCode += `    address: "${loc.address}",\n`;
  tsCode += `    phone: "${loc.phone}",\n`;
  tsCode += `    image: "${loc.image}",\n`;
  tsCode += `  },\n`;
});
tsCode += `];`;

const fs = require('fs');
fs.writeFileSync('c:/Users/veera/Hyundai Website/newLocations.ts', tsCode);
console.log('done');
